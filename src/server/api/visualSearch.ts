...
import { embedImage, imageUrlToBase64 } from "@/lib/jinaEmbedding";
 ...
 * Admin-only endpoint that processes question/answer images
 */
export async function indexQuestions({
  ...
  for (const q of questions) {
    ...
    for (let i = 0; i < questionImages.length; i++) {
      try {
        ...
        const embedding = await embedImage(imageBase64, jinaApiKey);
  ...
  return {
    success: true,
    progress,
    message: `Indexed ${progress.indexed} images from ${questions.length} questions`,
  };
}
...
export async function searchByImage({
  ...
  try {
    queryEmbedding = await embedImage(imageBase64, jinaApiKey);
  } catch (error) {
